...
default_url="http://10.201.238.11/${path}"
...
echo "create user"
sudo mysql -p=$sudopw --execute="CREATE USER '$dbuser'@'localhost' IDENTIFIED WITH mysql_native_password BY '$dbpass';"
...
${RESET}"
wp core download --path=/var/www/$path
cd $path
...
sudo find /var/www/$path -type d -exec chmod 750 {} \;
sudo find /var/www/$path -type f -exec chmod 640 {} \;
